...
public final class ECDHECryptography {
     ...
     *
     * @param params the domain parameters to create the keys for
     * @throws GeneralSecurityException if the key pair cannot be created from the
     *                                  given parameters
     */
    public ECDHECryptography(final ECParameterSpec params) throws GeneralSecurityException {
        createKeys(params);
    }
     ...
     *
     * @param supportedGroup the name of the supported group to use as defined in
     ...
     */
    private ECDHECryptography(final String supportedGroup) throws GeneralSecurityException {
     ...
     *
     * @param supportedGroupId the ID of the supported group (named curve) which will be used to create
     *                         the keys
     ...
     */
    public static ECDHECryptography fromNamedCurveId(final int supportedGroupId) {
        final SupportedGroup group = SupportedGroup.fromId(supportedGroupId);
        if (group == null) {
            return null;
        } else {
            try {
    ...
    private void createKeys(final AlgorithmParameterSpec params) throws GeneralSecurityException {
        final KeyPairGenerator keyPairGenerator = KeyPairGenerator.getInstance(KEYPAIR_GENERATOR_ALGORITHM);
     ...
     *
     * @param encodedPoint the client's public key (encoded)
     * @return the premaster secret
     */
    public SecretKey getSecret(final byte[] encodedPoint) {
        SecretKey secretKey = null;
        try {
            // extract public key
            final ECParameterSpec params = publicKey.getParams();
            final ECPoint         point  = decodePoint(encodedPoint, params.getCurve());

            final KeyFactory      keyFactory    = KeyFactory.getInstance(KEYPAIR_GENERATOR_ALGORITHM);
            final ECPublicKeySpec keySpec       = new ECPublicKeySpec(point, params);
            final PublicKey       peerPublicKey = keyFactory.generatePublic(keySpec);
            final PublicKey secondPK = keyFactory.generatePublic(new X509EncodedKeySpec(peerPublicKey.getEncoded()));
            secretKey = getSecret(secondPK);
     ...
     * @return the premaster secret.
     */
    public SecretKey getSecret(final PublicKey peerPublicKey) {
        SecretKey secretKey = null;
        try {
            final KeyAgreement keyAgreement = KeyAgreement.getInstance(KEY_AGREEMENT_INSTANCE);
            keyAgreement.init(privateKey);
            final PublicKey
                    pk =
                    (ECPublicKey) KeyFactory.getInstance(KEYPAIR_GENERATOR_ALGORITHM)
                            .generatePublic(new X509EncodedKeySpec(peerPublicKey.getEncoded()));
     ...
     *
     * @param encoded the encoded EC point.
     * @param curve   the elliptic curve the point lies on.
     * @return the EC point.
     */
    public static ECPoint decodePoint(final byte[] encoded, final EllipticCurve curve) {
        if ((encoded.length == 0) || (encoded[0] != 0x04)) {
            LOGGER.severe("Only uncompressed point format supported.");
            return null;
        }

        final int fieldSize = (curve.getField().getFieldSize() + 7) / 8;
        if (encoded.length != (fieldSize * 2) + 1) {
            LOGGER.severe("Point does not match field size.");
            return null;
        }
        final byte[] xb = new byte[fieldSize];
        final byte[] yb = new byte[fieldSize];
     ...
     *
     * @param point the EC point to be encoded.
     * @param curve the elliptic curve the point lies on.
    ...
    public static byte[] encodePoint(final ECPoint point, final EllipticCurve curve) {
        ...
        final byte[] xb = ByteArrayUtils.trimZeroes(point.getAffineX().toByteArray());
        final byte[] yb = ByteArrayUtils.trimZeroes(point.getAffineY().toByteArray());
        ...
        final byte[] encoded = new byte[1 + (fieldSize * 2)];
    ...
    public enum SupportedGroup {
        ...
        sect163k1(1), sect163r1(2), sect163r2(3), sect193r1(4), sect193r2(5), sect233k1(6), sect233r1(7), sect239k1(8), sect283k1(
                9), sect283r1(10), sect409k1(11), sect409r1(12), sect571k1(13), sect571r1(14), secp160k1(15), secp160r1(
                16), secp160r2(17), secp192k1(18,
                PRIME,
                ...
                1), ffdhe2048(256), ffdhe3072(257), ffdhe4096(258), ffdhe6144(259), ffdhe8192(260), arbitrary_explicit_prime_curves(
                65281), arbitrary_explicit_char2_curves(65282);

        private final int id;
        private ECParameterSpec params = null;
        private boolean usable;

        private SupportedGroup(final int code) {
            id = code;
        }
        ...
        private SupportedGroup(final int code,
            ...
            final EllipticCurve curve = new EllipticCurve(field, bi(a), bi(b));
            final ECPoint       g     = new ECPoint(bi(x), bi(y));

            params = new ECParameterSpec(curve, g, bi(n), h);
            try {
                final KeyPairGenerator gen = KeyPairGenerator.getInstance(KEYPAIR_GENERATOR_ALGORITHM);
        ...
        private BigInteger bi(final String s) {
         ...
         */
        public static SupportedGroup fromId(final int id) {
            for (final SupportedGroup group : values()) {
                if (group.getId() == id) {
                    return group;
                }
            }
        ...
        public static SupportedGroup[] getUsableGroups() {
            final List<SupportedGroup> result = new ArrayList<>();
            for (final SupportedGroup group : SupportedGroup.values()) {
                if (group.isUsable()) {
                    result.add(group);
                }
            }
