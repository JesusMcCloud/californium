...
public class ServerHandshaker extends Handshaker {
     ...
     * @param sessionListener     the listener to notify about the session's life-cycle events.
     * @param config              the DTLS configuration.
     * @param maxTransmissionUnit the MTU value reported by the network interface the record layer is bound to.
     * @throws HandshakeException   if the handshaker cannot be initialized
     * @throws NullPointerException if session or recordLayer is <code>null</code>.
     */
    public ServerHandshaker(final DTLSSession session,
                            final RecordLayer recordLayer,
                            final SessionListener sessionListener,
                            final DtlsConnectorConfig config,
                            final int maxTransmissionUnit) throws HandshakeException {
     ...
     * following the full DTLS handshake protocol.
     *
     * @param initialMessageSequenceNo the initial message sequence number to expect from the peer
     ...
     * @param config                   the DTLS configuration.
     * @param maxTransmissionUnit      the MTU value reported by the network interface the record layer is bound to.
     * @throws IllegalStateException    if the message digest required for computing the FINISHED message hash cannot be instantiated.
     * @throws IllegalArgumentException if the <code>initialMessageSequenceNo</code> is negative.
     * @throws NullPointerException     if session, recordLayer or config is <code>null</code>.
     */
    public ServerHandshaker(final int initialMessageSequenceNo,
        ...
        supportedCipherSuites = Arrays.asList(config.getSupportedCipherSuites());

        pskStore = config.getPskStore();

        privateKey = config.getPrivateKey();
        certificateChain = config.getCertificateChain();
        publicKey = config.getPublicKey();

        clientAuthenticationRequired = config.isClientAuthenticationRequired();

        supportedClientCertificateTypes = new ArrayList<>();
        supportedClientCertificateTypes.add(CertificateType.RAW_PUBLIC_KEY);
        if (rootCertificates != null && rootCertificates.length > 0) {
            final int index = config.isSendRawKey() ? 1 : 0;
            supportedClientCertificateTypes.add(index, CertificateType.X_509);
        }

        supportedServerCertificateTypes = new ArrayList<>();
        if (privateKey != null && publicKey != null) {
            if (certificateChain == null || certificateChain.length == 0) {
                supportedServerCertificateTypes.add(CertificateType.RAW_PUBLIC_KEY);
            } else if (config.isSendRawKey()) {
                supportedServerCertificateTypes.add(CertificateType.RAW_PUBLIC_KEY);
                supportedServerCertificateTypes.add(CertificateType.X_509);
            } else {
                supportedServerCertificateTypes.add(CertificateType.X_509);
                supportedServerCertificateTypes.add(CertificateType.RAW_PUBLIC_KEY);
            }
    ...
    @Override
    protected synchronized void doProcessMessage(final DTLSMessage message) throws
                                                                            HandshakeException,
        ...
        if (lastFlight != null) {
            ...
            LOGGER.log(Level.FINER,
                    "Received client's ({0}) FINISHED message again, retransmitting last flight...",
                    getPeerAddress());
        ...
        if (LOGGER.isLoggable(Level.FINE)) {
            ...
            msg.append(String.format("Processing %s message from peer [%s]",
        ...
        switch (message.getContentType()) {
            case CHANGE_CIPHER_SPEC:
                ...
                LOGGER.log(Level.FINE,
                        "Processed {1} message from peer [{0}]",
                        new Object[]{message.getPeer(), message.getContentType()});
                break;

            case HANDSHAKE:
                recordLayer.cancelRetransmissions();
                final HandshakeMessage handshakeMsg = (HandshakeMessage) message;
                ...
                switch (handshakeMsg.getMessageType()) {
                    ...
                    case CLIENT_KEY_EXCHANGE:
                        ...
                        switch (getKeyExchangeAlgorithm()) {
                            ...
                            default:
                                throw new HandshakeException(String.format("Unsupported key exchange algorithm %s",
                                        getKeyExchangeAlgorithm().name()),
                                        new AlertMessage(AlertLevel.FATAL,
                                                AlertDescription.HANDSHAKE_FAILURE,
                                                handshakeMsg.getPeer()));
                        }
                        handshakeMessages =
                                ByteArrayUtils.concatenate(handshakeMessages, clientKeyExchange.getRawMessage());

                        if (!clientAuthenticationRequired ||
                            getKeyExchangeAlgorithm() != KeyExchangeAlgorithm.EC_DIFFIE_HELLMAN) {
                    ...
                    default:
                        throw new HandshakeException(String.format("Received unexpected %s message from peer %s",
                                handshakeMsg.getMessageType(),
                                handshakeMsg.getPeer()),
                                new AlertMessage(AlertLevel.FATAL,
                ...
                LOGGER.log(Level.FINE,
                        "Processed {1} message with message sequence no [{2}] from peer [{0}]",
     ...
     */
    private void receivedCertificateVerify(final CertificateVerify message) throws HandshakeException {
     ...
     *                            cannot be created
     */
    private void receivedClientFinished(final Finished message) throws HandshakeException {
        if (lastFlight != null) {
            return;
        }
        ...
        final DTLSFlight flight = new DTLSFlight(getSession());
		 ...
		 */
        final ChangeCipherSpecMessage changeCipherSpecMessage = new ChangeCipherSpecMessage(session.getPeer());
        ...
        final Finished finished = new Finished(getMasterSecret(), isClient, handshakeHash, session.getPeer());
    ...
    private void receivedClientHello(final ClientHello clientHello) throws HandshakeException {
		 ...
		 */
        final ServerHelloDone serverHelloDone = new ServerHelloDone(session.getPeer());
    ...
    private void createServerHello(final ClientHello clientHello, final DTLSFlight flight) throws HandshakeException {
        ...
        final ProtocolVersion serverVersion = negotiateProtocolVersion(clientHello.getClientVersion());
        ...
        if (!clientHello.getCompressionMethods().contains(CompressionMethod.NULL)) {
            ...
            throw new HandshakeException("Client does not support NULL compression method",
                    new AlertMessage(AlertLevel.FATAL, AlertDescription.HANDSHAKE_FAILURE, clientHello.getPeer()));
        } else {
            session.setCompressionMethod(CompressionMethod.NULL);
        }

        final HelloExtensions serverHelloExtensions = new HelloExtensions();
        negotiateCipherSuite(clientHello, serverHelloExtensions);

        final MaxFragmentLengthExtension maxFragmentLengthExt = clientHello.getMaxFragmentLengthExtension();
        ...
        final ServerNameExtension serverNameExt = clientHello.getServerNameExtension();
    ...
    private void createCertificateMessage(final ClientHello clientHello, final DTLSFlight flight) throws
                                                                                                  HandshakeException {
    ...
    private void createServerKeyExchange(final ClientHello clientHello, final DTLSFlight flight) throws
                                                                                                 HandshakeException {
        ...
        switch (getKeyExchangeAlgorithm()) {
            case EC_DIFFIE_HELLMAN:
                ...
                signatureAndHashAlgorithm =
                        new SignatureAndHashAlgorithm(SignatureAndHashAlgorithm.HashAlgorithm.SHA256,
                                SignatureAndHashAlgorithm.SignatureAlgorithm.ECDSA);
                try {
                    ecdhe = new ECDHECryptography(negotiatedSupportedGroup.getEcParams());
                    serverKeyExchange =
                            new ECDHServerKeyExchange(signatureAndHashAlgorithm,
                                    ecdhe,
                                    privateKey,
                                    clientRandom,
                                    serverRandom,
                                    negotiatedSupportedGroup.getId(),
                                    session.getPeer());
                    break;
                } catch (final GeneralSecurityException e) {
                    e.printStackTrace();
                    throw new HandshakeException(String.format("Error performing EC Diffie Hellman key exchange: %s",
                            e.getMessage()),
    ...
    private void createCertificateRequest(final ClientHello clientHello, final DTLSFlight flight) throws
                                                                                                  HandshakeException {

        if (clientAuthenticationRequired && signatureAndHashAlgorithm != null) {

            final CertificateRequest certificateRequest = new CertificateRequest(session.getPeer());

            // TODO make this variable, reasonable values
            certificateRequest.addCertificateType(ClientCertificateType.ECDSA_SIGN);
            certificateRequest.addSignatureAlgorithm(new SignatureAndHashAlgorithm(signatureAndHashAlgorithm.getHash(),
                    signatureAndHashAlgorithm.getSignature()));
     ...
     */
    private byte[] receivedClientKeyExchange(final ECDHClientKeyExchange message) {
        clientKeyExchange = message;
        final byte[] premasterSecret = ecdhe.getSecret(message.getEncodedPoint()).getEncoded();
     ...
     * @param message the client's key exchange message.
     * @return the premaster secret
     * @throws HandshakeException if no specified preshared key available.
     */
    private byte[] receivedClientKeyExchange(final PSKClientKeyExchange message) throws HandshakeException {
        ...
        if (psk == null) {
            throw new HandshakeException(String.format("Cannot authenticate client, identity [%s] is unknown",
                    identity),
                    new AlertMessage(AlertLevel.FATAL, AlertDescription.HANDSHAKE_FAILURE, session.getPeer()));
        } else {
     ...
     * @return the premaster secret
     */
    private byte[] receivedClientKeyExchange(final NULLClientKeyExchange message) {
        clientKeyExchange = message;
    ...
    public void startHandshake() throws HandshakeException {
        final HelloRequest helloRequest = new HelloRequest(session.getPeer());
     ...
     */
    private ProtocolVersion negotiateProtocolVersion(final ProtocolVersion clientVersion) throws HandshakeException {
        final ProtocolVersion version = new ProtocolVersion();
        if (clientVersion.compareTo(version) >= 0) {
            return new ProtocolVersion();
        } else {
            final AlertMessage
                    alert =
                    new AlertMessage(AlertLevel.FATAL, AlertDescription.PROTOCOL_VERSION, session.getPeer());
     ...
     * </p>
     *
     * @param clientHello           the <em>CLIENT_HELLO</em> message containing the list of cipher suites
     *                              the client supports (ordered by preference).
     * @param serverHelloExtensions the container object to add server extensions to that are required for the selected
     *                              cipher suite.
     * @throws HandshakeException if this server's configuration does not support any of the cipher suites
     *                            proposed by the client.
     */
    private void negotiateCipherSuite(final ClientHello clientHello, final HelloExtensions serverHelloExtensions) throws
                                                                                                                  HandshakeException {

        final CertificateType supportedServerCertType = getSupportedServerCertificateType(clientHello);
        final CertificateType supportedClientCertType = getSupportedClientCertificateType(clientHello);
        final SupportedGroup  group                   = negotiateNamedCurve(clientHello);

        for (final CipherSuite cipherSuite : clientHello.getCipherSuites()) {
            ...
            if (cipherSuite != CipherSuite.TLS_NULL_WITH_NULL_NULL && supportedCipherSuites.contains(cipherSuite)) {
                if (isEligible(cipherSuite, supportedServerCertType, supportedClientCertType, group)) {
                    ...
                    LOGGER.log(Level.FINER,
                            "Negotiated cipher suite [{0}] with peer [{1}]",
    ...
    private boolean isEligible(final CipherSuite cipher,
                               final CertificateType supportedServerCertType,
                               final CertificateType supportedClientCertType,
                               final SupportedGroup group) {
        boolean result = true;
    ...
    private void addServerHelloExtensions(final CipherSuite negotiatedCipherSuite, final HelloExtensions extensions) {
        if (negotiatedClientCertificateType != null) {
            ...
            final ClientCertificateTypeExtension ext = new ClientCertificateTypeExtension(false);
        ...
        if (negotiatedServerCertificateType != null) {
            ...
            final ServerCertificateTypeExtension ext = new ServerCertificateTypeExtension(false);
        ...
        if (negotiatedCipherSuite.isEccBased()) {
            ...
            final List<ECPointFormat> formats = Arrays.asList(ECPointFormat.UNCOMPRESSED);
     ...
     *
     * @param clientHello the peer's <em>CLIENT_HELLO</em> message containing its
     *                    preferred elliptic curves
     * @return the selected curve or {@code null} if server and client have no curves in common
     */
    private static SupportedGroup negotiateNamedCurve(final ClientHello clientHello) {
        SupportedGroup                         result          = null;
        final List<SupportedGroup>             preferredGroups = SupportedGroup.getPreferredGroups();
        final SupportedEllipticCurvesExtension extension       = clientHello.getSupportedEllipticCurvesExtension();
        if (extension == null) {
            ...
            for (final Integer preferredGroupId : extension.getSupportedGroupIds()) {
                // use first group proposed by client contained in list of server's preferred groups
                final SupportedGroup group = SupportedGroup.fromId(preferredGroupId);
    ...
    private CertificateType getSupportedClientCertificateType(final ClientHello clientHello) throws HandshakeException {
        ...
        final ClientCertificateTypeExtension certTypeExt = clientHello.getClientCertificateTypeExtension();
        if (certTypeExt != null) {
            for (final CertificateType certType : certTypeExt.getCertificateTypes()) {
    ...
    private CertificateType getSupportedServerCertificateType(final ClientHello clientHello) throws HandshakeException {
        final ServerCertificateTypeExtension certTypeExt = clientHello.getServerCertificateTypeExtension();
        if (certTypeExt != null) {
            for (final CertificateType certType : certTypeExt.getCertificateTypes()) {
    ...
    protected boolean isFirstMessageReceived(final HandshakeMessage handshakeMessage) {
        if (HandshakeType.CLIENT_HELLO.equals(handshakeMessage.getMessageType())) {
            final Random messageRandom = ((ClientHello) handshakeMessage).getRandom();
