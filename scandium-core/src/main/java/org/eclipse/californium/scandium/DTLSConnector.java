...
public class DTLSConnector implements Connector {
    ...
    private static final Logger
                             LOGGER                            =
            Logger.getLogger(DTLSConnector.class.getCanonicalName());
    ...
    private static final int
            ...
            CipherSuite.TLS_PSK_WITH_AES_128_CBC_SHA256.getMaxCiphertextExpansion();
    // CBC cipher has largest expansion
    private static final int
                             MAX_DATAGRAM_BUFFER_SIZE          =
            MAX_PLAINTEXT_FRAGMENT_LENGTH + 12 // DTLS message headers
    ...
    private final Object          errorHandleLock = new Object();
    ...
    private final SessionListener sessionCacheSynchronization;
     ...
     */
    public DTLSConnector(final DtlsConnectorConfig configuration) {
    ...
    public DTLSConnector(final DtlsConnectorConfig configuration, final SessionCache sessionCache) {
        this(configuration,
                new InMemoryConnectionStore(configuration.getMaxConnections(),
    ...
    DTLSConnector(final DtlsConnectorConfig configuration, final ResumptionSupportingConnectionStore connectionStore) {
        ...
        if (configuration == null) {
            ...
            outboundMessages = new LinkedBlockingQueue<>(config.getOutboundMessageBufferSize());
            this.connectionStore = connectionStore;
            sessionCacheSynchronization = (SessionListener) this.connectionStore;
        }
    }
     ...
     */
    public final synchronized void setExecutor(final StripedExecutorService executor) {
     ...
     */
    public final void close(final InetSocketAddress peerAddress) {
    ...
    private void start(final InetSocketAddress bindAddress) throws IOException {
        ...
        timer =
                Executors.newSingleThreadScheduledExecutor(new DaemonThreadFactory("DTLS-Retransmit-Task-",
        ...
        if (executor == null) {
            ...
            if (threadCount == 1) {
                ...
                executor =
                        Executors.newSingleThreadExecutor(new DaemonThreadFactory("DTLS-Connection-Handler-",
                                NamedThreadFactory.SCANDIUM_THREAD_GROUP));
            } else {
                executor =
                        new StripedExecutorService(Executors.newFixedThreadPool(threadCount,
                                new DaemonThreadFactory("DTLS-Connection-Handler-",
                                        NamedThreadFactory.SCANDIUM_THREAD_GROUP)));
            }

            hasInternalExecutor = true;
        }
        ...
        if (lastBindAddress != null &&
            (!socket.getLocalAddress().equals(lastBindAddress.getAddress()) ||
             socket.getLocalPort() != lastBindAddress.getPort())) {
        ...
        final NetworkInterface ni = NetworkInterface.getByInetAddress(bindAddress.getAddress());
        if (ni != null && ni.getMTU() > 0) {
            maximumTransmissionUnit = ni.getMTU();
        } else {
            LOGGER.config("Cannot determine MTU of network interface, using minimum MTU [1280] of IPv6 instead");
            maximumTransmissionUnit = 1280;
        }

        if (config.getMaxFragmentLengthCode() != null) {
            final MaxFragmentLengthExtension.Length
                    lengthCode =
                    MaxFragmentLengthExtension.Length.fromCode(config.getMaxFragmentLengthCode());
        ...
        receiver = new Worker("DTLS-Receiver-" + lastBindAddress) {
            ...
            private final byte[] receiverBuffer = new byte[DTLSConnector.this.inboundDatagramBufferSize];
     ...
     */
    public final synchronized void forceResumeSessionFor(final InetSocketAddress peer) {
        final Connection peerConnection = connectionStore.get(peer);
    ...
    private void receiveNextDatagramFromNetwork(final DatagramPacket packet) throws IOException {
        ...
        final InetSocketAddress peerAddress = new InetSocketAddress(packet.getAddress(), packet.getPort());

        final byte[]       data    = Arrays.copyOfRange(packet.getData(), packet.getOffset(), packet.getLength());
        final List<Record> records = Record.fromByteArray(data, peerAddress);
        LOGGER.log(Level.FINER,
                "Received {0} DTLS records using a {1} byte datagram buffer",
    ...
    private void processRecord(final Record record) {
        ...
        try {
            ...
            LOGGER.log(Level.INFO,
                    String.format("Unexpected error occurred while processing record from peer [%s]",
     ...
     */
    private void terminateOngoingHandshake(final InetSocketAddress peerAddress,
        ...
        if (connection != null && connection.hasOngoingHandshake()) {
            ...
            final DTLSSession  session    = handshaker.getSession();
            final AlertMessage alert      = new AlertMessage(AlertLevel.FATAL, description, peerAddress);
    ...
    private void terminateConnection(final InetSocketAddress peerAddress) {
        if (peerAddress != null) {
            terminateConnection(connectionStore.get(peerAddress));
        }
    }

    private void terminateConnection(final Connection connection) {
        if (connection != null) {
     ...
     *                   <code>null</code>)
     */
    private void terminateConnection(final Connection connection, final AlertMessage alert, final DTLSSession session) {
        ...
        if (alert == null) {
            ...
            LOGGER.log(Level.FINE,
                    "Terminating connection with peer [{0}], reason [{1}]",
            ...
            synchronized (session) {
                ...
                if (session.isRecordProcessable(record.getEpoch(), record.getSequenceNumber())) {
                    try {
                        ...
                        final ApplicationMessage message = (ApplicationMessage) record.getFragment();
                    ...
                    LOGGER.log(Level.FINER,
                            "Discarding duplicate APPLICATION_DATA record received from peer [{0}]",
                            record.getPeerAddress());
                }
            }
    ...
    private RawData createApplicationMessage(final ApplicationMessage message, final DTLSSession session) {
        final DtlsEndpointContext
                context =
                new DtlsEndpointContext(message.getPeer(),
     ...
     */
    private void processAlertRecord(final Record record) {

        final Connection connection = connectionStore.get(record.getPeerAddress());
        if (connection == null) {
            LOGGER.log(Level.FINER,
                    "Discarding ALERT record from [{0}] received without existing connection",
                    record.getPeerAddress());
        } else {
    ...
    private void processAlertRecord(final Record record, final Connection connection) {
        ...
        if (connection.hasEstablishedSession() &&
            ...
            processAlertRecord(record, connection, connection.getEstablishedSession());
                   ...
                   connection.getOngoingHandshake().getSession().getReadEpoch() == record.getEpoch()) {
    ...
    private void processAlertRecord(final Record record, final Connection connection, final DTLSSession session) {
        record.setSession(session);
        try {
            final AlertMessage alert      = (AlertMessage) record.getFragment();
            final Handshaker   handshaker = connection.getOngoingHandshake();
    ...
    private void processChangeCipherSpecRecord(final Record record) {
        final Connection connection = connectionStore.get(record.getPeerAddress());
        if (connection != null && connection.hasOngoingHandshake()) {
            ...
            LOGGER.log(Level.FINE,
                    "Received CHANGE_CIPHER_SPEC record from peer [{0}] with no handshake going on",
                    record.getPeerAddress());
        }
    }
    ...
    private void processHandshakeRecordWithoutConnection(final Record record) throws HandshakeException {
        if (record.getEpoch() > 0) {
            ...
            try {
                ...
                final HandshakeMessage handshakeMessage = (HandshakeMessage) record.getFragment();
     ...
     */
    private void processHandshakeRecordWithConnection(final Record record, final Connection connection) throws
                                                                                                        HandshakeException {
        if (connection.hasOngoingHandshake()) {
            final DTLSSession handshakeSession = connection.getOngoingHandshake().getSession();
    ...
    private void processDecryptedHandshakeMessage(final HandshakeMessage handshakeMessage,
                                                  final Record record,
     ...
     * @throws HandshakeException if the handshake message cannot be processed
     */
    private static void processOngoingHandshakeMessage(final HandshakeMessage message,
                                                       final Record record,
                                                       final Connection connection) throws HandshakeException {
        if (connection.hasOngoingHandshake()) {
            connection.getOngoingHandshake().processMessage(record);
     ...
     */
    private void processHelloRequest(final HelloRequest helloRequest, final Connection connection) throws
                                                                                                   HandshakeException {
        if (connection.hasOngoingHandshake()) {
    ...
    private void processClientHello(final ClientHello clientHello, final Record record) throws HandshakeException {
        if (LOGGER.isLoggable(Level.FINE)) {
            final StringBuilder
                    msg =
                    new StringBuilder("Processing CLIENT_HELLO from peer [").append(record.getPeerAddress())
                            .append("]");
            if (LOGGER.isLoggable(Level.FINEST)) {
     ...
     *                            to start a new handshake or resume an existing session
     */
    private void processClientHello(final ClientHello clientHello,
                                    final Record record,
                                    final Connection connection) throws HandshakeException {
        if (LOGGER.isLoggable(Level.FINE)) {
            final StringBuilder
    ...
    private static boolean isHandshakeAlreadyStartedForMessage(final ClientHello clientHello,
                                                               final Connection connection) {
        return connection != null &&
               connection.hasOngoingHandshake() &&
     ...
     */
    private boolean isClientInControlOfSourceIpAddress(final ClientHello clientHello, final Record record) {
        ...
        try {
            final byte[] expectedCookie = cookieGenerator.generateCookie(clientHello);
            ...
            throw new DtlsHandshakeException("Cannot compute cookie for peer",
    ...
    private void startNewHandshake(final ClientHello clientHello, final Record record) throws HandshakeException {
        final Connection peerConnection = new Connection(record.getPeerAddress());
        ...
        final DTLSSession newSession = new DTLSSession(record.getPeerAddress(), false, record.getSequenceNumber());
        // initialize handshaker based on CLIENT_HELLO (this accounts
        // for the case that multiple cookie exchanges have taken place)
        final Handshaker
                handshaker =
                new ServerHandshaker(clientHello.getMessageSeq(),
                        newSession,
                        getRecordLayerForPeer(peerConnection),
                        peerConnection,
                        config,
                        maximumTransmissionUnit);
        addSessionCacheSynchronization(handshaker);
    ...
    private void resumeExistingSession(final ClientHello clientHello, final Record record) throws HandshakeException {
        LOGGER.log(Level.FINER,
                "Client [{0}] wants to resume session with ID [{1}]",
        ...
        if (previousConnection != null && previousConnection.isActive()) {
            ...
            final DTLSSession
                    sessionToResume =
                    new DTLSSession(clientHello.getSessionId(),
                            record.getPeerAddress(),
                            ticket,
            ...
            final Handshaker
                    handshaker =
                    new ResumingServerHandshaker(clientHello.getMessageSeq(),
                            sessionToResume,
                            getRecordLayerForPeer(peerConnection),
                            peerConnection,
                            config,
                    ...
                    handshaker.addSessionListener(new SessionAdapter() {
                        ...
                        public void sessionEstablished(final Handshaker currentHandshaker,
                            ...
                            LOGGER.log(Level.FINER,
                                    ...
                                    new Object[]{previousConnection.getPeerAddress(),
    ...
    private void sendHelloVerify(final ClientHello clientHello, final Record record, final byte[] expectedCookie) {
        // send CLIENT_HELLO_VERIFY with cookie in order to prevent
        // DOS attack as described in DTLS 1.2 spec
        LOGGER.log(Level.FINER,
                "Verifying client IP address [{0}] using HELLO_VERIFY_REQUEST",
                record.getPeerAddress());
        final HelloVerifyRequest
                msg =
                new HelloVerifyRequest(new ProtocolVersion(), expectedCookie, record.getPeerAddress());
        ...
        final Record
                helloVerify =
                new Record(ContentType.HANDSHAKE, 0, record.getSequenceNumber(), msg, record.getPeerAddress());
        try {
    ...
    void send(final AlertMessage alert, final DTLSSession session) {
        if (alert == null) {
            ...
            try {
                sendRecord(new Record(ContentType.ALERT,
    ...
    public final void send(final RawData msg) {
        if (msg == null) {
            ...
            throw new IllegalArgumentException("Message data must not exceed " +
                                               MAX_PLAINTEXT_FRAGMENT_LENGTH +
                                               " bytes");
        } else {
            final boolean queueFull = !outboundMessages.offer(msg);
            if (queueFull) {
                LOGGER.log(Level.WARNING,
                        "Outbound message queue is full! Dropping outbound message to peer [{0}]",
    ...
    private void sendNextMessageOverNetwork() throws HandshakeException {
        ...
        try {
            final RawData message = outboundMessages.take(); // Blocking
            sendMessage(message);
    ...
    private void sendMessage(final RawData message) throws HandshakeException {
        ...
        final InetSocketAddress peerAddress = message.getInetSocketAddress();
        ...
        if (session == null) {
            ...
            final Handshaker
                    handshaker =
                    new ClientHandshaker(new DTLSSession(peerAddress, true),
                            getRecordLayerForPeer(connection),
                            connection,
                            config,
                            maximumTransmissionUnit);
        ...
        // TODO what if there already is an ongoing handshake with the peer
            ...
            final DTLSSession
                    resumableSession =
                    new DTLSSession(session.getSessionIdentifier(), peerAddress, session.getSessionTicket(), 0);

            // terminate the previous connection and add the new one to the store
            final Connection newConnection = new Connection(peerAddress);
            terminateConnection(connection, null, null);
            connectionStore.put(newConnection);
            final Handshaker
                    handshaker =
                    new ResumingClientHandshaker(resumableSession,
                            getRecordLayerForPeer(newConnection),
                            newConnection,
                            config,
                            maximumTransmissionUnit);
            addSessionCacheSynchronization(handshaker);
    ...
    private void sendMessage(final RawData message, final DTLSSession session) {
        try {
            final EndpointContext
                    ctx =
                    new DtlsEndpointContext(message.getInetSocketAddress(),
                            session.getPeerIdentity(),
                            session.getSessionIdentifier().toString(),
            ...
            final Record
                    record =
                    new Record(ContentType.APPLICATION_DATA,
                            session.getWriteEpoch(),
                            session.getSequenceNumber(),
            ...
            LOGGER.log(Level.FINE,
                    String.format("Cannot send APPLICATION record to peer [%s]", message.getInetSocketAddress()),
                    e);
            message.onError(e);
        }
    }
    ...
    private boolean checkOutboundEndpointContext(final RawData message, final EndpointContext connectionContext) {
        ...
        if (null != endpointMatcher && !endpointMatcher.isToBeSent(message.getEndpointContext(), connectionContext)) {
            if (LOGGER.isLoggable(Level.WARNING)) {
                LOGGER.log(Level.WARNING,
                        "DTLSConnector ({0}) drops {1} bytes to {2}:{3}",
                        new Object[]{this, message.getSize(), message.getAddress(), message.getPort()});
            }
    ...
    private SessionListener newDeferredMessageSender(final RawData message) {
        return new SessionAdapter() {
            ...
            @Override
            public void sessionEstablished(final Handshaker handshaker, final DTLSSession establishedSession) throws
                                                                                                              HandshakeException {
                LOGGER.log(Level.FINE,
                        "Session with [{0}] established, now sending deferred message",
                        establishedSession.getPeer());
                sendMessage(message, establishedSession);
            }

            @Override
            public void handshakeFailed(final InetSocketAddress peer, final Throwable error) {
     ...
     */
    public final DTLSSession getSessionByAddress(final InetSocketAddress address) {
        if (address == null) {
            return null;
        }
        final Connection connection = connectionStore.get(address);
        if (connection != null) {
    ...
    private void sendHandshakeFlight(final DTLSFlight flight, final Connection connection) {
        if (flight != null) {
    ...
    private void sendFlight(final DTLSFlight flight) {
        ...
        final List<DatagramPacket> datagrams = new ArrayList<>();

        try {
            for (final Record record : flight.getMessages()) {
                ...
                if (payload.length + recordBytes.length > maxDatagramSize) {
                    ...
                    final DatagramPacket
                            ...
                            new DatagramPacket(payload,
                                    payload.length,
                                    flight.getPeerAddress().getAddress(),
            ...
            LOGGER.log(Level.FINER,
                    "Sending flight of {0} message(s) to peer [{1}] using {2} datagram(s) of max. {3} bytes",
                    new Object[]{flight.getMessages().size(),
                                 flight.getPeerAddress(),
                                 datagrams.size(),
                                 maxDatagramSize});
            for (final DatagramPacket datagramPacket : datagrams) {
    ...
    private void sendRecord(final Record record) throws IOException {
        final byte[]         recordBytes = record.toByteArray();
        final DatagramPacket datagram    = new DatagramPacket(recordBytes, recordBytes.length, record.getPeerAddress());
        sendDatagram(datagram);
    }

    private void sendDatagram(final DatagramPacket datagramPacket) throws IOException {
        final DatagramSocket socket = getSocket();
    ...
    private void handleTimeout(final DTLSFlight flight) {
        ...
        if (flight.getTries() < max) {
            LOGGER.log(Level.FINE,
                    "Re-transmitting flight for [{0}], [{1}] retransmissions left",
            ...
            LOGGER.log(Level.FINE,
                    "Flight for [{0}] has reached maximum no. [{1}] of retransmissions, discarding ...",
                    new Object[]{flight.getPeerAddress(), max});
            // inform handshaker
            final Connection connection = connectionStore.get(flight.getPeerAddress());
            if (null != connection) {
                final Handshaker handshaker = connection.getOngoingHandshake();
                if (null != handshaker) {
    ...
    private void scheduleRetransmission(final DTLSFlight flight) {
        ...
        if (flight.isRetransmissionNeeded()) {
            ...
            final ScheduledFuture<?>
                    f =
                    timer.schedule(new RetransmitTask(flight), flight.getTimeout(), TimeUnit.MILLISECONDS);
     ...
     */
    public final int getMaximumFragmentLength(final InetSocketAddress peer) {
        final Connection con = connectionStore.get(peer);
    ...
    private class RetransmitTask implements Runnable {
        ...
        private final DTLSFlight flight;
    ...
    @Override
    public synchronized void setEndpointContextMatcher(final EndpointContextMatcher endpointContextMatcher) {
    ...
    private void connectionClosed(final InetSocketAddress peerAddress) {
        if (peerAddress != null) {
            connectionStore.remove(peerAddress);
        }
    }

    private void handleExceptionDuringHandshake(final Throwable cause,
                                                final AlertLevel level,
                                                final AlertDescription description,
                                                final Record record) {
