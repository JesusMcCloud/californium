...
public final class ECDHServerKeyExchange extends ServerKeyExchange {
    ...
    private static final Logger
                                LOGGER                 =
            Logger.getLogger(ECDHServerKeyExchange.class.getCanonicalName());
    ...
    private final int curveType = NAMED_CURVE;
     ...
     * @param ecdhe                     the ECDHE helper class
     ...
     * @param namedCurveId              the named curve's id which will be used for the ECDH
     ...
     */
    public ECDHServerKeyExchange(final SignatureAndHashAlgorithm signatureAndHashAlgorithm,
                                 final ECDHECryptography ecdhe,
                                 final PrivateKey serverPrivateKey,
                                 final Random clientRandom,
                                 final Random serverRandom,
                                 final int namedCurveId,
                                 final InetSocketAddress peerAddress) throws GeneralSecurityException {

        this(signatureAndHashAlgorithm, namedCurveId, peerAddress);

        publicKey = ecdhe.getPublicKey();
        final ECParameterSpec parameters = publicKey.getParams();
        ...
        final Signature signature = Signature.getInstance(this.signatureAndHashAlgorithm.jcaName());
     ...
     *
     * @param signatureAndHashAlgorithm the algorithm to use
     * @param curveId                   the named curve index
     * @param pointEncoded              the point on the curve (encoded)
     ...
     */
    private ECDHServerKeyExchange(final SignatureAndHashAlgorithm signatureAndHashAlgorithm,
                                  final int curveId,
        ...
        final SupportedGroup group = SupportedGroup.fromId(curveId);
        if (group == null || !group.isUsable()) {
            throw new HandshakeException(String.format("Server used unsupported elliptic curve (%d) for ECDH", curveId),
            ...
            try {
                ...
                final KeyFactory keyFactory = KeyFactory.getInstance(KEYPAIR_GENERATOR_INSTANCE);
                final ECPublicKey
                        tmp =
                        (ECPublicKey) keyFactory.generatePublic(new ECPublicKeySpec(point, group.getEcParams()));
                publicKey = (ECPublicKey) keyFactory.generatePublic(new X509EncodedKeySpec(tmp.getEncoded()));
            } catch (final GeneralSecurityException e) {
                LOGGER.log(Level.FINE, "Cannot re-create server's public key from params", e);
                throw new HandshakeException(String.format("Cannot re-create server's public key from params: %s",
                        e.getMessage()),
    ...
    private void writeNamedCurve(final DatagramWriter writer) {
    ...
    public static HandshakeMessage fromByteArray(final byte[] byteArray, final InetSocketAddress peerAddress) throws
                                                                                                              HandshakeException {
        final DatagramReader reader    = new DatagramReader(byteArray);
        final int            curveType = reader.read(CURVE_TYPE_BITS);
        switch (curveType) {
    ...
    private static ECDHServerKeyExchange readNamedCurve(final DatagramReader reader,
                                                        final InetSocketAddress peerAddress) throws HandshakeException {
        final int    curveId      = reader.read(NAMED_CURVE_BITS);
        int          length       = reader.read(PUBLIC_LENGTH_BITS);
        final byte[] pointEncoded = reader.readBytes(length);

        final byte[] bytesLeft = reader.readBytesLeft();
        ...
        if (bytesLeft.length > 0) {
            final DatagramReader remainder          = new DatagramReader(bytesLeft);
            final int            hashAlgorithm      = remainder.read(HASH_ALGORITHM_BITS);
            final int            signatureAlgorithm = remainder.read(SIGNATURE_ALGORITHM_BITS);
    ...
    public int getMessageLength() {
        ...
        switch (curveType) {
            ...
            case NAMED_CURVE:
                ...
                final int signatureLength = (signatureEncoded == null) ? 0 : 2 + 2 + signatureEncoded.length;
     ...
     */
    public void verifySignature(final PublicKey serverPublicKey,
                                final Random clientRandom,
                                final Random serverRandom) throws HandshakeException {
        if (signatureEncoded == null) {
        ...
        try {
            final Signature signature = Signature.getInstance(signatureAndHashAlgorithm.jcaName());
        ...
        if (!verified) {
            final String message = "The server's ECDHE key exchange message's signature could not be verified.";
            final AlertMessage
                    alert =
                    new AlertMessage(AlertLevel.FATAL, AlertDescription.HANDSHAKE_FAILURE, getPeer());
     ...
     */
    private void updateSignature(final Signature signature, final Random clientRandom, final Random serverRandom) throws
                                                                                                                  SignatureException {
    ...
    private void updateSignatureForNamedCurve(final Signature signature) throws SignatureException {
